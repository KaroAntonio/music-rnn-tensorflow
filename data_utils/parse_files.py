...
def read_wav_as_np(filename):
    ...
    #Normalize 16-bit input to [0, 2] range
...
def convert_wav_files_to_nptensor(directory, block_size, max_seq_len, out_file, max_files=20, useTimeDomain=False):
    ...
    if(num_files > max_files):
        print('Limiting Input File Batch to: '+max_files)
        num_files = max_files
    ...
    np.save(out_file+'_tensor', x_data)
...
def load_training_example(filename, block_size=2048, useTimeDomain=False):
    ...
    if useTimeDomain:
        return x_t, y_t
    #take mel spectrum instead maybe
    X = np.array(time_blocks_to_fft_blocks(x_t))
    Y = np.array(time_blocks_to_fft_blocks(y_t))
    return X, Y
